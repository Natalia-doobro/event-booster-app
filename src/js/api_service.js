...
export async function fetchEventsByName(query, page) {
  const {
    ...
    `size=20&keyword=${query}&page=${page}&apikey=1twKLyrauG3OZrFZiN9ApTE1ANWFyZTo`,
  );
  return events;
}
...
export async function fetchPopularEvents(page, classification, country) {
  const {
    ...
    `size=20&page=${page}&sort=random&classificationName=${classification}&marketId=${country}&apikey=1twKLyrauG3OZrFZiN9ApTE1ANWFyZTo`,
  );
  return events;
}
